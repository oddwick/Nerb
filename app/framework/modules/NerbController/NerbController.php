...
abstract class NerbController
{
    ...
    public function __construct( string $mode, int $node, int $offset = 0 )
    {
        ...
        if( 
	        ...
	        throw new NerbError( "The value <code>[$mode]</code> is not a valid url mode.  Expecting <code>[REST|KEY-VALUE|KEYWORD|QSA]</code>" );
        }
        ...
        switch( $mode ){
	        case 'rest':
		        $this->url = new NerbUrlRest( $this->controller, $node, $offset );
		        break;
	        
	        case 'key-value':
		        $this->url = new NerbUrlKeyValue( $this->controller, $node, $offset );
		        break;
	        
	        case 'keyword':
		        $this->url = new NerbUrlKeyword( $this->controller, $node, $offset );
		        break;
	        
	        default:
		        $this->url = new NerbUrlQsa( $this->controller, $node, $offset );
        }// end switch
