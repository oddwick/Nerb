...
class NerbLog
{
    ...
    protected function init() : bool
    {
        ...
        if ( !$status ) 
            throw new NerbError( 'Unable to initialize log file <code>['.$this->log_file.']</code>');
    ...
    protected function backupLogfile() : bool
    {
        ...
        if( !$status ) throw new NerbError('Could not move log file <code>['.$this->log_file.']</code>');
        ...
        if( COMPRESS_LOG_BACKUP ){
            ...
            if( !$status ) throw new NerbError('Error compressing backup log. Make sure your permissions are properly set');
        }
