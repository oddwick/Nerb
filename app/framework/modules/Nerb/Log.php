...
class Log
{
    ...
    protected function init() : bool
    {
        ...
        if ( !$status ) 
            throw new Error( 'Unable to initialize log file <code>['.$this->log_file.']</code>');
    ...
    protected function backupLogfile() : bool
    {
        ...
        if( !$status ) throw new Error('Could not move log file <code>['.$this->log_file.']</code>');
        ...
        if( COMPRESS_LOG_BACKUP ){
            ...
            if( !$status ) throw new Error('Error compressing backup log. Make sure your permissions are properly set');
        }
    ...
    public function write( string $message, string $prefix = null ) : bool
    {
        ...
        if ( !$status ) 
            throw new Error( 'Unable to add entry to log');
